...
def method_tfa02_ifa19_3(appln_cntxt, db_session, mongo_client, control_params_dict):
    ...
    # this will get the database from the URI string
        ...
        # in  order to get the value back in the local scope need to pass the scope to exec call, else returned_op_str is not resolved.
        d=locals()
        dynamic_execution_to_stringify_call = exec(call_to_get_execution_str, None, d)
        returned_op_str = d['returned_op_str']

        print('returned_op_str', d['returned_op_str'])
        ...
        with mongo_client.start_session() as session:
            dynamic_execution_to_execute_on_mongo= exec('mongo_cursor_op='+returned_op_str)
