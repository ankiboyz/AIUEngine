...
def prepare_playground_clear_vestige(appln_cntxt, db_session, mongo_client, control_params_dict):
    ''' This method is used to work as Stage 1 for the control.
     Wherein it prepares the Playground for the execution of the control. Resets any left overs so that they can be
     re-processed.
     '''
...
def mark_the_ones_to_be_handled(appln_cntxt, db_session, mongo_client, control_params_dict):
    ...
    # this will get the database from the URI string
        ...
        mongo_pipeline_code_str = list_of_mongo_statements.AG_PIPELINE_GENERIC_STEP_3_MARK_ONES_TO_PROCESS
...
def filling_up_exceptionId_in_exception_collection(appln_cntxt, db_session, mongo_client, control_params_dict):
    ''' This method is used to work as Stage 5 for the control.
     Additional Processing over the prepared Exceptions; here in the exceptionIDs are filled up with the
     string value of the exception ID.
      '''
...
def flag_recs_in_function_collection_as_processed(appln_cntxt, db_session, mongo_client, control_params_dict):
...
def whether_all_function_recs_done_decisional_node(appln_cntxt, db_session, mongo_client, control_params_dict):
